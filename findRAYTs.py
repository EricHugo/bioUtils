...
class parseMapFile():
    def __init__(self, map_file, query_column, query_selection):
        self.map_file = map_file
        self.query_column = query_column
        self.query_selection = query_selection
        gene_files = open(self.map_file, 'r+b')
        self.gene_mem = mmap.mmap(gene_files(), 0, prot=mmap.PROT_READ)
        headers = self.gene_mem.readline().split()
        self.col = 0
        for header in headers:
            if header == self.query_column:
                break
            self.col += 1
    ...
    def __next__(self):
        # here sort of what column to return
        return gene_paths()

    def gene_paths(self):
        """Extracts and outputs iterable of all gene pathes matching query in 
        column"""
        for mapping in self.gene_mem.split():
...
def main():
    ...
    for i in range(5):
        parseMapFile("reference_results.tsv", "Genus", "Shewanella")
    sys.exit()
